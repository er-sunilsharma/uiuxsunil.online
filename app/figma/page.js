import React from "react";
import Link from "next/link";
import { LuFigma } from "react-icons/lu";
import { SiAdobephotoshop } from "react-icons/si";

export default function About() {
  return (
    <>
      <div className="w-full bg-gray-50 pt-4 md:pt-20 pb-10 md:pb-20">
        <div className="mx-auto max-w-6xl px-4 space-y-10 md:space-y-24">
          <div className="w-full">
            <div className="w-full text-2xl font-semibold pb-5 md:pb-10">
              Application Design
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-8 items-center">
              <div className="w-full bg-white rounded-2xl pt-5 pb-7 px-4 text-center shadow-2xl">
                <div className="w-full mb-4">
                  <h2 className="text-lg font-medium">
                    Super Kirana App Dashboard
                  </h2>
                </div>
                <Link
                  target="_blank"
                  rel="nofollow"
                  href="https://www.figma.com/proto/IhwObTdps693GzqwZ8QEqy/Super-Kirana-App-Dashboard?page-id=0%3A1&node-id=12-95&starting-point-node-id=12%3A95&t=uOn0p8VNhIh8dPxu-1"
                  class="inline-flex overflow-hidden text-white bg-gray-900 rounded-full group"
                >
                  <span class="px-3.5 py-2 text-white bg-red-600 group-hover:bg-purple-600 flex items-center justify-center">
                    <LuFigma size={24} />
                  </span>
                  <span class="pl-4 pr-5 py-2.5 font-medium">Figma Design</span>
                </Link>
              </div>
              <div className="w-full bg-white rounded-2xl pt-5 pb-7 px-4 text-center shadow-2xl">
                <div className="w-full mb-4">
                  <h2 className="text-lg font-medium">Vaccination India App</h2>
                </div>
                <Link
                  target="_blank"
                  rel="nofollow"
                  href="https://www.figma.com/proto/aGeVD5u8BiOHy3EMgMMCEU/Vaccination-India-App?page-id=0%3A1&node-id=1-9&viewport=570%2C1824%2C0.4&t=O04TA4Jjo7VzbjBi-1&scaling=contain&content-scaling=fixed&starting-point-node-id=1%3A9"
                  class="inline-flex overflow-hidden text-white bg-gray-900 rounded-full group"
                >
                  <span class="px-3.5 py-2 text-white bg-red-600 group-hover:bg-purple-600 flex items-center justify-center">
                    <LuFigma size={24} />
                  </span>
                  <span class="pl-4 pr-5 py-2.5 font-medium">Figma Design</span>
                </Link>
              </div>
              <div className="w-full bg-white rounded-2xl pt-5 pb-7 px-4 text-center shadow-2xl">
                <div className="w-full mb-4">
                  <h2 className="text-lg font-medium">
                    RedBus App Design Clone
                  </h2>
                </div>
                <Link
                  target="_blank"
                  rel="nofollow"
                  href="https://www.figma.com/proto/gDYFQLcOvyoht2yX4OEQkg/Online-Bus-Booking-App?page-id=0%3A1&node-id=1-2&viewport=233%2C371%2C0.96&t=MGuZDriqxHguO75d-1&scaling=scale-down&content-scaling=fixed"
                  class="inline-flex overflow-hidden text-white bg-gray-900 rounded-full group"
                >
                  <span class="px-3.5 py-2 text-white bg-red-600 group-hover:bg-purple-600 flex items-center justify-center">
                    <LuFigma size={24} />
                  </span>
                  <span class="pl-4 pr-5 py-2.5 font-medium">Figma Design</span>
                </Link>
              </div>
            </div>
          </div>
          <div className="w-full">
            <div className="w-full text-2xl font-semibold pb-5 md:pb-10">
              Template Design
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-8 items-center">
              <div className="w-full bg-white rounded-2xl pt-5 pb-7 px-4 text-center shadow-2xl">
                <div className="w-full mb-4">
                  <h2 className="text-lg font-medium">
                    BDA Media Official Website
                  </h2>
                </div>
                <Link
                  target="_blank"
                  rel="nofollow"
                  href="https://www.figma.com/proto/FSIl07pxhUVORFVC1hbo4n/BDA-Media?page-id=0%3A1&node-id=142-160&viewport=0%2C500%2C0.13&t=I6Um0lTJi3oTD6Tx-1&scaling=min-zoom&content-scaling=fixed"
                  class="inline-flex overflow-hidden text-white bg-gray-900 rounded-full group"
                >
                  <span class="px-3.5 py-2 text-white bg-red-600 group-hover:bg-purple-600 flex items-center justify-center">
                    <LuFigma size={24} />
                  </span>
                  <span class="pl-4 pr-5 py-2.5 font-medium">Figma Design</span>
                </Link>
              </div>
              <div className="w-full bg-white rounded-2xl pt-5 pb-7 px-4 text-center shadow-2xl">
                <div className="w-full mb-4">
                  <h2 className="text-lg font-medium">
                    Financial Market Template
                  </h2>
                </div>
                <Link
                  target="_blank"
                  rel="nofollow"
                  href="https://www.figma.com/proto/qr9nzQTZjvdQBZffzEIiXE/Best-Financial-Market?page-id=0%3A1&node-id=3-3&viewport=286%2C347%2C0.13&t=BU57QEGa61rAlyle-1&scaling=min-zoom&content-scaling=fixed"
                  class="inline-flex overflow-hidden text-white bg-gray-900 rounded-full group"
                >
                  <span class="px-3.5 py-2 text-white bg-red-600 group-hover:bg-purple-600 flex items-center justify-center">
                    <LuFigma size={24} />
                  </span>
                  <span class="pl-4 pr-5 py-2.5 font-medium">Figma Design</span>
                </Link>
              </div>
              <div className="w-full bg-white rounded-2xl pt-5 pb-7 px-4 text-center shadow-2xl">
                <div className="w-full mb-4">
                  <h2 className="text-lg font-medium">
                    Dark Empire Market Template
                  </h2>
                </div>
                <Link
                  target="_blank"
                  rel="nofollow"
                  href="https://www.figma.com/proto/qr9nzQTZjvdQBZffzEIiXE/Best-Financial-Market?page-id=0%3A1&node-id=27-2&viewport=286%2C347%2C0.13&t=BU57QEGa61rAlyle-1&scaling=min-zoom&content-scaling=fixed"
                  class="inline-flex overflow-hidden text-white bg-gray-900 rounded-full group"
                >
                  <span class="px-3.5 py-2 text-white bg-red-600 group-hover:bg-purple-600 flex items-center justify-center">
                    <LuFigma size={24} />
                  </span>
                  <span class="pl-4 pr-5 py-2.5 font-medium">Figma Design</span>
                </Link>
              </div>
              <div className="w-full bg-white rounded-2xl pt-5 pb-7 px-4 text-center shadow-2xl">
                <div className="w-full mb-4">
                  <h2 className="text-lg font-medium">
                    Financial Shopping Template
                  </h2>
                </div>
                <Link
                  target="_blank"
                  rel="nofollow"
                  href="https://www.figma.com/proto/qr9nzQTZjvdQBZffzEIiXE/Best-Financial-Market?page-id=0%3A1&node-id=31-358&viewport=286%2C347%2C0.13&t=BU57QEGa61rAlyle-1&scaling=min-zoom&content-scaling=fixed"
                  class="inline-flex overflow-hidden text-white bg-gray-900 rounded-full group"
                >
                  <span class="px-3.5 py-2 text-white bg-red-600 group-hover:bg-purple-600 flex items-center justify-center">
                    <LuFigma size={24} />
                  </span>
                  <span class="pl-4 pr-5 py-2.5 font-medium">Figma Design</span>
                </Link>
              </div>
              <div className="w-full bg-white rounded-2xl pt-5 pb-7 px-4 text-center shadow-2xl">
                <div className="w-full mb-4">
                  <h2 className="text-lg font-medium">
                    Financial Market Template 02
                  </h2>
                </div>
                <Link
                  target="_blank"
                  rel="nofollow"
                  href="https://www.figma.com/proto/qr9nzQTZjvdQBZffzEIiXE/Best-Financial-Market?page-id=0%3A1&node-id=45-597&viewport=286%2C347%2C0.13&t=BU57QEGa61rAlyle-1&scaling=min-zoom&content-scaling=fixed"
                  class="inline-flex overflow-hidden text-white bg-gray-900 rounded-full group"
                >
                  <span class="px-3.5 py-2 text-white bg-red-600 group-hover:bg-purple-600 flex items-center justify-center">
                    <LuFigma size={24} />
                  </span>
                  <span class="pl-4 pr-5 py-2.5 font-medium">Figma Design</span>
                </Link>
              </div>
            </div>
          </div>
          <div className="w-full">
            <div className="w-full text-2xl font-semibold pb-5 md:pb-10">
              Logo Design
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-8 items-center">
              <div className="w-full bg-white rounded-2xl pt-5 pb-7 px-4 text-center shadow-2xl">
                <div className="w-full mb-4">
                  <h2 className="text-lg font-medium">Shadow Bazaar Logo</h2>
                </div>
                <Link
                  target="_blank"
                  rel="nofollow"
                  href="https://www.figma.com/design/RHflUaWdB4dzZC0f1FnxmQ/ShadowBazaar-Logo?node-id=0-1&t=ztTZo9VVfxVzyDb2-1"
                  class="inline-flex overflow-hidden text-white bg-gray-900 rounded-full group"
                >
                  <span class="px-3.5 py-2 text-white bg-red-600 group-hover:bg-purple-600 flex items-center justify-center">
                    <LuFigma size={24} />
                  </span>
                  <span class="pl-4 pr-5 py-2.5 font-medium">Figma Design</span>
                </Link>
              </div>
            </div>
          </div>
        </div>
      </div>
    </>
  );
}
